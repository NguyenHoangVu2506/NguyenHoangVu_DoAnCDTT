...
function Customer_List() {
    ...
    useEffect(() => {
        (async function () {
            await userservice.getAll('user').then((res) => {
                ...
                <div className="container-fluid">
                    <div className="row mb-2">
                        <div className="col-sm-12">
                            <h1 className="d-inline">Danh sách khách hàng <sup style={{ fontSize: "14px" }}>({count_user})</sup></h1>
                        </div>
                        <div className="col-sm-1 mt-2 text-right">
                            <Link className="action-btn" to="/admin/customer/trash" style={{ color: "red" }}>
                                        ...
                                        {users.map(function (user, index) {
                                            return (
                                                <tr className="datarow" key={index}>
                                                    ...
                                                    <td>
                                                        <Link to={`/admin/customer/update/${user.id}`} className="">
                                                            <FaEdit /> Update
                                                        </Link>
                                                        <Link to={`/admin/customer/show/${user.id}`} className="">
                                                            <FaEye /> Detail
                                                        </Link>
                                                        <button
                                                            onClick={() => {
